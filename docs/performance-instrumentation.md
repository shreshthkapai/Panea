...
User-facing behavior: when diagnostics.performance_overlay is enabled, Panea draws a compact in-window overlay with recent frame timing, renderer stats, PTY/parser throughput, atlas usage, and budget status.
Config keys: diagnostics.performance_overlay, renderer.gpu_timestamps, performance.max_frame_time_ms.
...
Fallback behavior: renderer.gpu_timestamps defaults to false. If enabled on an unsupported backend, the overlay reports timestamps unsupported and rendering continues.
...
Performance cost when disabled: diagnostics.performance_overlay returns before retaining samples or projecting scene overlays; scrollback memory estimates are skipped.
...
Tests: diagnostics overlay formatting tests, desktop disabled/enabled overlay projection tests, render-wgpu instrumentation tests, and standard layer/build checks.
```
...
- The overlay is intentionally developer-focused; polished placement, styling,
  toggle commands, and installed product UX remain later desktop polish.
- CI regression gates and reproducible cross-machine benchmark reporting remain
