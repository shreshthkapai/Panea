...
| Screenshot verification runner | tested | Deterministic renderer fixtures, including hidden/partial/visible/hover/no-controls fullscreen chrome states and a no-reflow pixel invariant, PPM capture, tolerance-based diffing, Windows baselines, and `cargo xtask screenshot verify --platform windows` exist. |
...
| Fullscreen chrome | tested | The client-owned overlay keeps native fullscreen state stable, routes controls through platform-neutral actions, preserves terminal/PTY geometry, uses bounded animation damage, and has deterministic Windows screenshot coverage. Final packaged interaction confirmation plus macOS/X11/Wayland native checklists remain. |
