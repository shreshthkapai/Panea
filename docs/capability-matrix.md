...
| Auto-hidden fullscreen titlebar | tested | implemented | implemented | implemented | Top-edge hover temporarily reveals the OS-owned decorated/maximized window and returns to borderless fullscreen on client re-entry. The terminal renderer draws no imitation chrome. Windows installed-build visual verification passes; macOS/X11/Wayland compositor verification remains. |
