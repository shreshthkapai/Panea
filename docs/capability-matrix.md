...
| Auto-hidden fullscreen titlebar | tested | implemented | implemented | implemented | Top-edge hover reveals batched Panea-owned chrome while the native window remains fullscreen. Terminal cells, pane geometry, and PTY size remain unchanged. Windows deterministic fixtures pass; packaged interaction confirmation and macOS/X11/Wayland native verification remain. |
...
| Screenshot verification | tested | partial | partial | partial | Deterministic fixtures, including five fullscreen chrome states and a no-reflow invariant, use PPM baselines, tolerance diffing, and reports. Windows baselines verify on the current host; macOS/Linux X11/Linux Wayland baselines remain uncaptured. |
